...
#define LEN 256
	...
	void AI(int a_board[8][8], int turn, int *sent_x, int *sent_y)
	{
		...
		for (i = 0; i < 8; i++) for (j = 0; j < 8; j++) if (a_board[i][j] == BLANK) tesuu--;
				...
				while (computer_cont){
					...
					if (computer){
						...
						*sent_x = computer % 10 - 1;
						...
						set_and_turn_over(board, player, computer / 10, computer % 10);
	...
	int saiki(int board[10][10], int player, int ai_player, int limit, int alpha, int beta){
		...
		int i, j, k, l, score, score_max = -9000, score_min = 9000;
		int boardcopy[10][10];
		int exist;
		...
		for (i = 1; i <= 8; i++){
			for (j = 1; j <= 8; j++){
				if (is_legal_move(board, player, i, j)){
					...
					score = saiki(board, player, ai_player, limit - 1, alpha, beta);
					printf("score:%d i;%d j:%d", score,i,j);
					for (k = 0; k<10; k++){
						for (l = 0; l<10; l++){
							board[k][l] = boardcopy[k][l];
						}
		...
		if (exist == OFF){
			return saiki(board, player, ai_player, limit, -beta, -alpha);
		}
	...
	int move(int board[10][10], int player, int ai_player, int limit){
		...
		for (i=1; i<=8; i++){
			for (j = 1; j <= 8; j++){
				if (is_legal_move(board, player, i, j)){
					set_and_turn_over(board, player, i, j);
					score = saiki(board, player, ai_player,limit, alpha, beta);
					printf("score:%d\n",score);
